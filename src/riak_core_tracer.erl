...
%% API
-export([start_link/0, stop/0, reset/0, filter/2, collect/0, collect/1, collect/2,
         results/0, stop_collect/0]).
...
-record(state,
        {trace = [], filters = [], mfs = [], stop_tref, stop_from, tracing = false}).
...
handle_call({filter, MFs, Filter}, _From, State) ->
  {reply,
   ok,
   State#state{mfs = MFs ++ State#state.mfs, filters = [Filter | State#state.filters]}};
handle_call({collect, Duration, Nodes}, _From, State) ->
  ...
  dbg:stop(),
  dbg:tracer(process,
             {fun ({trace, _, call, {?MODULE, trigger_sentinel, _}}, Pid) ->
